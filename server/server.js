...
function loadFormMetadata() {
  return readJson(FORM_METADATA_FILE, DEFAULT_FORM_METADATA);
}
...
// ---- Belge (PDF) kaydetme — base64 data URL'yi gerçek dosyaya yazar ----
function saveDocument(kind, key, dataUrl, originalName) {
  const match = /^data:application\/pdf;base64,(.+)$/.exec(dataUrl);
  if (!match) throw new Error("Sadece PDF (base64 data URL) kabul edilir.");
  const buffer = Buffer.from(match[1], "base64");
  if (buffer.length > 5 * 1024 * 1024) throw new Error("Dosya çok büyük (5MB üstü).");
  const filename = `${safeFileName(key)}.pdf`;
  const filePath = path.join(UPLOADS_DIR, kind, filename);
...
app.post("/api/reminders/snooze", (req, res) => {
  const until = req.body && req.body.until ? Number(req.body.until) : null;
  saveReminders({ snoozeUntil: until }).then(() => ok(res, { snoozeUntil: until }));
});
