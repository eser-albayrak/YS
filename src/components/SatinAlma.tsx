...
export default function SatinAlma({ onStartReception }: Props) {
  ...
  const [orderForm, setOrderForm] = useState<{
    ...
    teslimTarihi: new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString().slice(0, 10),
                ...
                {filteredOrders.map((po) => {
                  ...
                  return (
                    <tr key={po.id}>
                      ...
                      <td>{po.siparisTarihi}</td>
                      <td className="muted">{po.teslimTarihi || "—"}</td>
                      <td>
          ...
          <div className="card" style={{ width: "100%", maxWidth: "820px", maxHeight: "90vh", overflowY: "auto", background: "#18202c", border: "1px solid rgba(255,255,255,0.15)", padding: "20px" }}>
            ...
            <div className="grid2" style={{ gap: "12px" }}>
              ...
              <label>
                ...
                <input
                  type="date"
                  value={orderForm.siparisTarihi}
                  onChange={(e) => setOrderForm({ ...orderForm, siparisTarihi: e.target.value })}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="date"
                  value={orderForm.teslimTarihi}
                  onChange={(e) => setOrderForm({ ...orderForm, teslimTarihi: e.target.value })}
                />
              </label>
              ...
              <label>
                Ödeme Koşulları
                <input
                  value={orderForm.odemeKosullari}
                  onChange={(e) => setOrderForm({ ...orderForm, odemeKosullari: e.target.value })}
                  placeholder="örn. 30 Gün Vadeli, Peşin Havale"
                />
              </label>
              ...
              <label>
                Para Birimi
                <select
                  value={orderForm.paraBirimi}
                  onChange={(e) => setOrderForm({ ...orderForm, paraBirimi: e.target.value as any })}
                >
            ...
            {orderForm.kalemler.map((item, index) => (
              ...
              >
                ...
                <div className="grid2" style={{ gap: "10px" }}>
                  ...
                  <label>
                    Birim Fiyat ({orderForm.paraBirimi}) <span className="muted" style={{ fontWeight: "normal", fontSize: "0.8rem" }}>(İsteğe Bağlı)</span>
                    <input
                      type="number"
                      step="0.01"
                      placeholder="Boş kalırsa mektupta fiyat görünmez"
                      value={item.birimFiyat || ""}
                      onChange={(e) => handleOrderItemChange(item.id, "birimFiyat", e.target.value)}
                    />
                  </label>
                </div>

                <div style={{ textAlign: "right", marginTop: 8, fontSize: "0.9rem", color: "#60a5fa" }}>
                  {item.birimFiyat > 0 ? (
                    <>Tutar: <strong>{(item.toplamFiyat || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })} {orderForm.paraBirimi}</strong></>
                  ) : (
                    <span className="muted" style={{ fontStyle: "italic" }}>* Fiyat girilmedi (Sipariş mektubunda fiyat sütunu gizlenecektir)</span>
                  )}
                </div>
              </div>
            ))}
            ...
            {orderFormTotalCost > 0 && (
              <div style={{ background: "rgba(96, 165, 250, 0.1)", padding: "12px", borderRadius: "8px", textAlign: "right", fontWeight: 600, fontSize: "1.1rem" }}>
                GENEL TOPLAM TUTAR: {orderFormTotalCost.toLocaleString("tr-TR", { minimumFractionDigits: 2 })} {orderForm.paraBirimi}
              </div>
            )}
            ...
            <div className="po-printable-sheet" ref={poPrintRef} style={{ background: "#fff", padding: "10px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
                ...
                <div style={{ border: "1px solid #ccc", padding: "12px", borderRadius: "4px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#333", marginTop: "2px" }}><strong>Tahmini Teslimat:</strong> {viewPo.teslimTarihi || "—"}</div>
                </div>
              </div>
