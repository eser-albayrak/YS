        ...
        <div className="card material-form">
          ...
          <div className="grid2">
            ...
            <label>
              Firma
              <select
                value={suppliers.includes(form.firma || "") ? form.firma : (form.firma ? "__custom__" : "")}
                onChange={(e) => {
              ...
              >
                <option value="">-- Dropdown Listesinden Seçin --</option>
                {suppliers.map((s) => (
                  <option key={s} value={s}>
                    🏢 {s}
                  </option>
                ))}
                {form.firma && !suppliers.includes(form.firma) && (
                  <option value="__custom__">✏️ Özel: {form.firma}</option>
                )}
              </select>

              <input
                list="material-form-firma-list"
                value={form.firma || ""}
                onChange={(e) => setForm({ ...form, firma: e.target.value })}
                placeholder="Veya firma adını buraya yazın..."
              />
              <datalist id="material-form-firma-list">
                {suppliers.map((s) => (
                  <option key={s} value={s} />
                ))}
              </datalist>
