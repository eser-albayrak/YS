  ...
  return (
    <div className="label-wrap">
      <div className="card no-print">
        ...
        <button className="btn-primary" disabled={busy} onClick={handlePrint}>
          {packages.length > 1
...
function ReprintCard({
  ...
  const printDate = receipt.etiketBasimTarihi
    ? new Date(receipt.etiketBasimTarihi).toLocaleDateString("tr-TR")
    : "—";
...
const DEFAULT_SETTINGS: LabelSettings = {
  widthMm: 100,
  heightMm: 50,
  headerText: "B.R. LEVENT PLASTİK A.ŞRİŞİTE ONAYLI – DEPO KULLANIMI İÇİNDİR",
  footerText: "",
  fontSizePt: 10,
  qrSizeMm: 20,
};
      ...
      <div className="panel">
        ...
        {ready.length === 0 ? (
          ...
          <div className="labels-grid">
            {ready.map((r) => (
              <PendingCard key={r.id} receipt={r} onChanged={onChanged} settings={settings} />
            ))}
          </div>
        )}
      </div>
